...
- Prefer the cheapest specialist that can own the task. Use `bug-fixer` for known or reproducible failures, `mechanical-editor` for explicit repetitive edits, `test-writer` for focused test additions, `remote-docs-researcher` for current external docs, and read-only local agents for discovery, mapping, review, log triage, local docs, or diff summaries.
